...
class PortraitBlurParamsWidget(BaseParamsWidget):
    """Portrait mode: keep subject sharp, blur background (best-effort without ML)."""
    def __init__(self, parent=None):
        super().__init__(parent)
        layout = QVBoxLayout(self)
        layout.setContentsMargins(0, 0, 0, 0)

        layout.addWidget(QLabel("Background Blur Sigma:"))
        self.blur_sigma = QDoubleSpinBox()
        self.blur_sigma.setMinimum(0.1)
        self.blur_sigma.setMaximum(25.0)
        self.blur_sigma.setValue(6.0)
        self.blur_sigma.setSingleStep(0.5)
        layout.addWidget(self.blur_sigma)

        layout.addWidget(QLabel("Subject Sensitivity (higher = larger subject area):"))
        self.sensitivity = QDoubleSpinBox()
        self.sensitivity.setMinimum(0.1)
        self.sensitivity.setMaximum(5.0)
        self.sensitivity.setValue(1.0)
        self.sensitivity.setSingleStep(0.1)
        layout.addWidget(self.sensitivity)

        layout.addWidget(QLabel("Edge Feather (soft transition):"))
        self.feather_sigma = QDoubleSpinBox()
        self.feather_sigma.setMinimum(0.0)
        self.feather_sigma.setMaximum(20.0)
        self.feather_sigma.setValue(4.0)
        self.feather_sigma.setSingleStep(0.5)
        layout.addWidget(self.feather_sigma)

        layout.addStretch()

    def get_params(self) -> dict:
        return {
            "portrait_blur_sigma": self.blur_sigma.value(),
            "portrait_sensitivity": self.sensitivity.value(),
            "portrait_feather_sigma": self.feather_sigma.value(),
        }
...
class omar_hosnyControlsWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        operations = {
            ...
            "Portrait Mode (Background Blur)": PortraitBlurParamsWidget,
        }
...
class omar_hosnyImageModule(IImageModule):
    def __init__(self):
        super().__init__()
        self._controls_widget = None

    def get_name(self) -> str:
        return "omar_hosny Module"
    ...
    def create_control_widget(self, parent=None, module_manager=None) -> QWidget:
        if self._controls_widget is None:
            self._controls_widget = omar_hosnyControlsWidget(module_manager, parent)
    ...
    def process_image(self, image_data: np.ndarray, metadata: dict, params: dict) -> np.ndarray:
        ...
        if operation == "Gaussian Blur":
        ...
        elif operation == "Portrait Mode (Background Blur)":
            blur_sigma = params.get("portrait_blur_sigma", 6.0)
            sensitivity = params.get("portrait_sensitivity", 1.0)
            feather_sigma = params.get("portrait_feather_sigma", 4.0)

            img = processed_data.astype(float)

            # --- 1) Build a "subject likelihood" map (where edges/details are) ---
            if img.ndim == 3 and img.shape[2] in [3, 4]:
                gray = rgb2gray(img[:, :, :3])
            else:
                gray = img

            edge_map = skimage.filters.sobel(gray)
            # Smooth edges into blobs (saliency-like)
            saliency = skimage.filters.gaussian(edge_map, sigma=5.0, preserve_range=True)

            # Threshold: sensitivity controls how much we keep
            # Higher sensitivity => lower threshold => larger subject area
            t = np.percentile(saliency, max(50, 85 - 10 * sensitivity))
            mask = saliency >= t

            # Clean mask
            mask = skimage.morphology.remove_small_objects(mask, min_size=300)
            mask = skimage.morphology.remove_small_holes(mask, area_threshold=600)
            mask = skimage.morphology.binary_dilation(mask, skimage.morphology.disk(5))

            # Keep largest connected component (often the main subject)
            ...
            # Feather mask edges for nicer blend
            alpha = mask.astype(float)
            if feather_sigma > 0:
                alpha = skimage.filters.gaussian(alpha, sigma=feather_sigma, preserve_range=True)
         ...
         # --- 2) Blur background ---
            if img.ndim == 3 and img.shape[2] in [3, 4]:
                ...
                for c in range(img.shape[2]):
                    blurred[:, :, c] = skimage.filters.gaussian(
                        img[:, :, c], sigma=blur_sigma, preserve_range=True
                    )
                # Expand alpha to channels
                alpha_c = alpha[:, :, None]
            else:
                blurred = skimage.filters.gaussian(img, sigma=blur_sigma, preserve_range=True)
                alpha_c = alpha

            # --- 3) Blend: subject (sharp) + background (blurred) ---
            blended = alpha_c * img + (1.0 - alpha_c) * blurred

            # Clip to dtype range safely before your final .astype(...)
            if np.issubdtype(image_data.dtype, np.integer):
                info = np.iinfo(image_data.dtype)
                blended = np.clip(blended, info.min, info.max)
            else:
                blended = np.clip(blended, np.min(img), np.max(img))

            processed_data = blended
